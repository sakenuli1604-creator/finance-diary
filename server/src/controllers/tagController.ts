...
class TagController {
  ...
  async create(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const { name, color, isProject } = req.body;
      if (!name) {
        return res.status(400).json({ message: 'name is required' });
      }
      const tag = await tagService.create(req.userId!, { name, color, isProject });
  ...
  async update(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const { name, color, isProject } = req.body;
      const tag = await tagService.update(req.userId!, req.params.id, { name, color, isProject });
      res.json(tag);
    } catch (error) {
      next(error);
    }
